...
public class PullWidget extends View{
	...
	private class PullRunnable implements Runnable{
		...
		@Override
		public void run(){
			movingHeight += -movingHeight * 0.1F;
	...
	private class CirclingRunnable implements Runnable{
		...
		public void run(){
			...
			handler.postDelayed(circlingRunnable, 12);
		}
	}
	...
	private class BackRunnable implements Runnable{
		...
		public void run(){
			...
			if(degree < 359){
				handler.postDelayed(backRunnable, 12);
			}else{
				degree = 0;
	...
	public PullWidget(Context context, AttributeSet attrs, int defStyle){
		...
		paint.setColor(context.getResources().getColor(R.color.global_highlight_color));
		path = new Path();
		
		iconDrawable = context.getResources().getDrawable(R.drawable.icon_pullwidget);
		
		minimumHeight = context.getResources().getDimensionPixelOffset(R.dimen.widget_pullwidget_minimum_height);
		maximumHeight = minimumHeight * 3;
		minimumContentHeight = context.getResources().getDimensionPixelOffset(R.dimen.widget_pullwidget_minimum_content_height);
